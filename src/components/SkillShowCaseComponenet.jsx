...
const skills = [
  ...
  },
  {
    ...
    color: "from-cyan-500/90 to-teal-500/90",
  },
  {
    id: "JavaScript",
    name: "JavaScript",
...
const SkillCard = ({ skill, isSelected, onClick, index }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`group relative w-full overflow-hidden rounded-2xl bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 p-4 sm:p-6 cursor-pointer transition-all duration-300 hover:bg-slate-800/80 hover:border-slate-600/60 hover:shadow-lg hover:shadow-slate-900/20 ${
        isSelected ? 'ring-2 ring-blue-400/50 bg-slate-800/80 border-slate-600/60' : ''
      }`}
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <h3 className="text-base sm:text-lg font-semibold text-slate-100 group-hover:text-white transition-colors">
              {skill.name}
            </h3>
          </div>
          {/* Show percentage only on mobile */}
          <motion.div 
            className="lg:hidden text-xs sm:text-sm font-medium text-slate-400 bg-slate-700/50 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full"
        ...
        <div className="lg:hidden mt-3 sm:mt-4 w-full bg-slate-700/50 rounded-full h-1 sm:h-1.5 overflow-hidden">
          <motion.div 
    ...
    >
      ...
      <div className="relative w-48 h-48 sm:w-56 sm:h-56">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          {skillName && (
            <motion.div 
              className="text-slate-300 text-sm sm:text-lg font-medium text-center px-4"
      ...
      {level > 0 && (
        ...
        >
          <span className="text-slate-300 text-sm sm:text-lg font-medium bg-slate-800/50 px-3 py-1 sm:px-4 sm:py-2 rounded-full border border-slate-700/50">
  ...
  return (
    <div className="min-h-screen px-4 py-16 sm:py-20">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <motion.div 
            className="inline-flex items-center space-x-2 bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 rounded-full px-4 py-2 mb-6"
            initial={{ opacity: 0, scale: 0.8 }}
          ...
          >
            ...
            />
            <span className="text-slate-300 text-sm sm:text-base font-medium">Available for opportunities</span>
          </motion.div>
          
          <motion.h1 
            className="text-3xl sm:text-5xl lg:text-6xl font-bold text-white mb-4 sm:mb-6 tracking-tight"
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            Technical
            <motion.span 
              className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400"
          ...
          <motion.p 
            className="text-slate-300 text-base sm:text-lg max-w-3xl mx-auto leading-relaxed font-light"
            initial={{ opacity: 0, y: 20 }}
            animate={isHeaderInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
        ...
        <div className="lg:hidden">
          ...
          >
            ...
            <div className="text-slate-400 text-xs sm:text-sm bg-slate-800/50 px-2.5 py-1 rounded-full border border-slate-700/50">
        ...
        <div className="hidden lg:grid lg:grid-cols-5 gap-12 items-start">
          ...
          <div className="lg:col-span-2 flex flex-col items-center lg:sticky lg:top-8">
            <motion.div 
              className="w-full bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 rounded-3xl p-8 mb-6"
            ...
            {!currentSkill && (
              <motion.div 
                className="w-full bg-slate-800/30 backdrop-blur-sm rounded-2xl border border-slate-700/30 p-6"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
              ...
              >
                <div className="text-center">
                  <motion.div 
                    className="w-12 h-12 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-xl flex items-center justify-center mx-auto mb-3"
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            >
              ...
              <motion.div 
                className="text-slate-400 text-sm bg-slate-800/50 px-3 py-1.5 rounded-full border border-slate-700/50"
